...
function worker_logic<WorkerPayload>() {
  ...
  parentPort.on("message", (message: ToWorkerMessage<WorkerPayload>) => {
    if (message.type === ToWorkerMessageType.DoWork) {
      ...
      (main as (...args: any) => Promise<any>)(message.payload.task_data).then((result) =>
...
export class ETP<WorkerPayload, WorkerResult> {
  ...
  constructor(
    ...
    protected worker_main: (data: WorkerPayload) => Promise<WorkerResult>,
    protected additional_deps = "",
  ) {
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      const worker_code =
        readFileSync(__filename)
          ...
          .replace(`"use` + ` deps";`, this.additional_deps) + `\n worker_logic();`;
